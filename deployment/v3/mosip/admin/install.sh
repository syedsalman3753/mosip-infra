...
function installing_admin() {
  ...
  helm -n $NS install admin-hotlist mosip/admin-hotlist --set-string nodeSelector.vlan="200" --version $CHART_VERSION

  echo Installing admin service. Will wait till service gets installed.
  helm -n $NS install admin-service mosip/admin-service --set-string nodeSelector.vlan="200" --set istio.corsPolicy.allowOrigins\[0\].prefix=https://$ADMIN_HOST --wait --version $CHART_VERSION

  echo Installing admin-ui
  helm -n $NS install admin-ui mosip/admin-ui --set-string nodeSelector.vlan="200" --set admin.apiUrl=https://$API_HOST/v1/ --set istio.hosts\[0\]=$ADMIN_HOST --version $CHART_VERSION
