...
function installing_resident() {
  ...
  helm -n $NS install resident nira/resident --set istio.corsPolicy.allowOrigins\[0\].prefix=https://$RESIDENT_HOST --version $CHART_VERSION -f ../ha-values.yaml $ENABLE_INSECURE

  echo Installing Resident UI
  helm -n $NS install resident-ui nira/resident-ui --set resident.apiHost=$API_HOST --set istio.hosts\[0\]=$RESIDENT_HOST --version $RESIDENT_UI_CHART_VERSION
