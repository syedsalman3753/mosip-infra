...
if [ $yn = "Y" ]
  then
    ...
    helm -n $NS install config-server mosip/config-server --set image.pullPolicy="IfNotPresent" --set-string nodeSelector.vlan="200" -f values.yaml --wait --version $CHART_VERSION
